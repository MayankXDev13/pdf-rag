...
def query_command(args):
    result = ask(
        question=args.question, k=args.k, filename=args.filename, show_sources=True
    )
...
def main():
    ...
    ingest_parser.add_argument(
        "--rebuild", action="store_true", help="Rebuild index for file"
    )
    ...
    query_parser.add_argument(
        "--k", type=int, default=3, help="Number of chunks to retrieve"
    )
    query_parser.add_argument("--filename", help="Filter by specific file")
    query_parser.add_argument(
        "--show-sources", action="store_true", default=True, help="Show sources"
    )
